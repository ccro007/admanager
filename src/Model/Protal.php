...
class Protal extends Model
{
    ...
    public function getRows($ids, $columns = null)
    {
        ...
        if ($ids) {
            ...
            $rows = $this->select(array('id' => $ids))->toArray();
    ...
    /**
     * Get one protal by id
     *
     * @param $id Protal id
     * @param null $columns Columns, null for default
     * @return array
     */
    public function getOneRow($id, $columns = null)
    {
        $rows =  $this->getRows(array($id), $columns);
        return  !empty($rows[$id]) ? $rows[$id] : array();
    }
